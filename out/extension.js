...
function getProfilesDir() {
    ...
    return c.get('profilesDir') || DEFAULT_PROFILES_DIR;
}
function getSettingsFile() {
    const c = vscode.workspace.getConfiguration('claudeSwitch');
    return c.get('settingsFile') || DEFAULT_SETTINGS_FILE;
}
function getLocalSettingsFile() {
    const c = vscode.workspace.getConfiguration('claudeSwitch');
    return c.get('localSettingsFile') || DEFAULT_LOCAL_SETTINGS_FILE;
}
...
function detectCurrentProfile(profiles) {
...
function applyProfile(filePath) {
    ...
    if (!writeJsonFile(getSettingsFile(), result)) {
        return false;
    }
    ...
    if (localSettings) {
        ...
        if (changed) {
            writeJsonFile(getLocalSettingsFile(), localSettings);
        }
    }
    return true;
}
function buildProfileData(profiles, current) {
    return {
        current: current ? {
            name: current.name,
            model: current.model,
...
class ClaudeSwitchProvider {
    ...
    async handleMessage(msg, webview) {
        switch (msg.type) {
            case 'ready': {
                this.pushData();
                break;
            ...
            case 'fetchModels': {
                ...
                // Try {baseUrl}/v1/models first. If 404, fall back to
                // {origin}/v1/models (e.g. DeepSeek's /anthropic path doesn't
                // serve /v1/models).
                const urlsToTry = [`${baseUrl}/v1/models`];
                try {
                ...
                for (const url of urlsToTry) {
                    try {
                        ...
                        if (!resp.ok) {
                            lastError = `API returned ${resp.status}: ${resp.statusText}`;
                            continue;
                        }
                        ...
                        const models = (json.data || [])
                            ...
                            .sort();
                        if (models.length === 0) {
                            lastError = 'No models returned from API.';
                            continue;
                        }
                        ...
                        lastError = `Failed to fetch models: ${e.message || e}`;
                    }
                }
            ...
            case 'switchProfile': {
                ...
                this.pushData();
                const action = await vscode.window.showInformationMessage(`Switched to profile "${name}". Reload window to apply environment changes?`, 'Reload Window', 'Later');
            ...
            case 'deleteProfile': {
                ...
                const confirm = await vscode.window.showWarningMessage(`Delete profile "${name}"? This cannot be undone.`, { modal: true }, 'Delete');
                if (confirm !== 'Delete') {
                    return;
                }
                try {
            ...
            case 'saveProfile': {
                const { name, isEdit, oldName, env } = msg;
                const filePath = path.join(getProfilesDir(), `${name}.json`);
                let profileData;
                if (isEdit && oldName) {
                    ...
                    if (fs.existsSync(filePath)) {
                        const confirm = await vscode.window.showWarningMessage(`Profile "${name}" already exists. Overwrite?`, { modal: true }, 'Overwrite');
...
// ---------------------------------------------------------------------------
let provider;
async function switchProfileHandler() {
    const profiles = listProfiles();
    const current = detectCurrentProfile(profiles);
    ...
    if (!selection || selection.profileName === current?.name) {
        return;
    }
    ...
    provider.pushData();
    const action = await vscode.window.showInformationMessage(`Switched to profile "${selection.profileName}". Reload window?`, 'Reload Window', 'Later');
    if (action === 'Reload Window') {
        await vscode.commands.executeCommand('workbench.action.reloadWindow');
    }
}
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('claude-switch.view', provider));
    context.subscriptions.push(vscode.commands.registerCommand('claude-switch.switchProfile', switchProfileHandler));
    context.subscriptions.push(vscode.commands.registerCommand('claude-switch.createProfile', () => {
